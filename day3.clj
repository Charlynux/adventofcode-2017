...
(defn neighbor-reducer [acc value]
  (let [prev (last acc)
    ...
    (conj acc {:position next-position
               :prev-move move
               :value (apply + (map :value neighbors))})))

;; Keeping my old code to show (sort of) progression.
(defn neighbor-step [acc]
  (let [prev (last acc)
        move (next-move (:position prev) (:prev-move prev))
        next-position (apply (move moves-fn) (:position prev))
        neighbors (find-neighbors next-position acc)]
...
#_(defn find-next-value-neighbors [number]
    (let [reducer-until
...
(def last-value (comp :value last))

(defn find-next-value-neighbors [number]
  (->>
   (iterate neighbor-step [init-state])
   (drop-while #(<= (last-value %) number))
   first
   last-value))
